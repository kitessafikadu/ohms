...
class ScssEditor(models.AbstractModel):
    ...
    @api.model
    def _clean_stale_muk_assets(self):
        stale_assets = self.env['ir.asset'].search([('path', 'like', '%muk_web_colors/%')])
        if stale_assets:
            stale_assets.unlink()
    ...
    def _get_color_variable(self, content, variable):
        value = re.search(fr'\$tt_{variable}\:?\s(.*?);', content)
